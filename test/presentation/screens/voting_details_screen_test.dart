...
void main() {
  ...
  group('VotingDetailsScreen', () {
    ...
    testWidgets('enables submit button when answer is selected', (tester) async {
      // Arrange
      when(() => mockVotingBloc.state).thenReturn(VotingInitial());
      when(() => mockVotingBloc.stream).thenAnswer((_) => const Stream.empty());
    ...
    testWidgets('shows confirmation dialog when submit is tapped', (tester) async {
      // Arrange
      when(() => mockVotingBloc.state).thenReturn(VotingInitial());
      when(() => mockVotingBloc.stream).thenAnswer((_) => const Stream.empty());
      when(() => mockVotingBloc.add(any())).thenAnswer((_) async {});
      ...
      expect(find.text('После подтверждения ваш голос будет засчитан, и изменить его будет нельзя.'), findsOneWidget);
      expect(find.text('Отмена'), findsOneWidget);
      expect(find.widgetWithText(ElevatedButton, 'Проголосовать'), findsNWidgets(2)); // One in screen, one in dialog
    });

    testWidgets('submits vote when confirmation dialog is confirmed', (tester) async {
      // Arrange
      when(() => mockVotingBloc.state).thenReturn(VotingInitial());
      when(() => mockVotingBloc.stream).thenAnswer((_) => const Stream.empty());
      when(() => mockVotingBloc.add(any())).thenAnswer((_) async {});
      ...
      // Confirm in dialog
      await tester.tap(find.widgetWithText(ElevatedButton, 'Проголосовать').last);
      await tester.pumpAndSettle();
    ...
    testWidgets('shows success dialog when vote is submitted successfully', (tester) async {
      // Arrange
      when(() => mockVotingBloc.state).thenReturn(VotingInitial());
    ...
    testWidgets('shows error snackbar when vote submission fails', (tester) async {
      // Arrange
      when(() => mockVotingBloc.state).thenReturn(VotingInitial());
    ...
    testWidgets('disables submit button when user already voted', (tester) async {
      // Arrange
      ...
      final submitButton = find.widgetWithText(ElevatedButton, 'Вы уже проголосовали');
      expect(tester.widget<ElevatedButton>(submitButton).onPressed, isNull);
    });

    testWidgets('shows snackbar when trying to submit without answering all questions', (tester) async {
      // Arrange
    ...
    testWidgets('renders empty state when no questions', (tester) async {
      ...
      // Assert
      expect(find.text('Вопросы для этого голосования отсутствуют.'), findsOneWidget);
    });
